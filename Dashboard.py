...
# ── Objetos de página (globales para poder pasarlos a st.switch_page) ──
...
def home_content():
    ...
    <style>
        ...
        * {{ font-family: 'Inter', sans-serif !important; }}
        ...
        .home-hero {{
            ...
            border-radius:24px; padding:72px 60px 68px; text-align:center;
            box-shadow:0 12px 56px rgba(40,5,63,0.40); position:relative; overflow:hidden;
            margin-bottom:40px; animation:fadeUp 0.6s ease both;
        }}
        .home-hero-eyebrow {{ display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,0.09);
            border:1px solid rgba(255,255,255,0.18);border-radius:99px;padding:5px 16px;
            font-size:11px;font-weight:700;color:rgba(255,255,255,0.75);letter-spacing:0.10em;
            text-transform:uppercase;margin-bottom:28px; }}
        .home-hero-dot {{ width:6px;height:6px;background:#34D399;border-radius:50%;animation:sbcPulse 1.8s ease-in-out infinite; }}
        .home-hero-title {{ font-size:48px;font-weight:900;color:white;margin:0 0 16px;letter-spacing:-1.5px;line-height:1.1; }}
        .home-hero-title span {{ background:linear-gradient(90deg,#38BDF8,#818CF8,#34D399);background-size:200% auto;
            -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:shimmer 3s linear infinite; }}
        .home-hero-sub {{ font-size:17px;color:rgba(255,255,255,0.58);max-width:560px;margin:0 auto 36px;line-height:1.65; }}
        .home-hero-pills {{ display:flex;justify-content:center;gap:10px;flex-wrap:wrap; }}
        .home-pill {{ background:rgba(255,255,255,0.10);border:1px solid rgba(255,255,255,0.20);border-radius:99px;
                      padding:6px 18px;font-size:12px;font-weight:600;color:rgba(255,255,255,0.75);letter-spacing:0.02em; }}
        .hero-orb {{ position:absolute;border-radius:50%;pointer-events:none; }}
        .hero-orb-1 {{ width:300px;height:300px;background:radial-gradient(circle,rgba(56,189,248,0.10) 0%,transparent 70%);top:-80px;left:-80px; }}
        .hero-orb-2 {{ width:200px;height:200px;background:radial-gradient(circle,rgba(129,140,248,0.12) 0%,transparent 70%);bottom:-60px;right:-60px; }}
        .home-section-lbl {{ font-size:10px;font-weight:800;color:#64748B;letter-spacing:0.14em;text-transform:uppercase;
            margin-bottom:14px;display:flex;align-items:center;gap:8px; }}
        .home-section-lbl::after {{ content:'';flex:1;height:1px;background:linear-gradient(90deg,#CBD5E1,transparent); }}

        /* Module cards */
        .mod-card {{ background:white;border-radius:22px;padding:36px 28px 28px;
            box-shadow:0 6px 32px rgba(0,0,0,0.09);border:1px solid rgba(0,0,0,0.04);
            position:relative;overflow:hidden;display:flex;flex-direction:column;
            transition:transform 0.28s ease,box-shadow 0.28s ease;animation:fadeUp 0.7s ease both; }}
        .mod-card:hover {{ transform:translateY(-6px);box-shadow:0 18px 56px rgba(0,0,0,0.14); }}
        .mod-card::before {{ content:'';position:absolute;top:0;left:0;right:0;height:5px;
            background:var(--mc,{COLOR_PRIMARY});border-radius:22px 22px 0 0; }}
        .mod-card::after {{ content:'';position:absolute;top:-40px;right:-40px;width:130px;height:130px;
            background:radial-gradient(circle,var(--mc,{COLOR_PRIMARY}),transparent 70%);opacity:0.10;border-radius:50%; }}
        .mod-card-wash {{ position:absolute;bottom:0;left:0;right:0;height:100px;
            background:linear-gradient(0deg,rgba(0,0,0,0.016),transparent);pointer-events:none; }}
        .mod-icon-wrap {{ width:68px;height:68px;border-radius:20px;display:flex;align-items:center;
            justify-content:center;font-size:34px;margin-bottom:20px;flex-shrink:0;
            position:relative;z-index:1;animation:float 3.5s ease-in-out infinite; }}
        .mod-status {{ display:inline-flex;align-items:center;gap:5px;font-size:9px;font-weight:700;
            padding:3px 10px;border-radius:99px;letter-spacing:0.08em;text-transform:uppercase;
            margin-bottom:12px;position:relative;z-index:1; }}
        .mod-title {{ font-size:22px;font-weight:900;color:#1E293B;margin:0 0 10px;
            letter-spacing:-0.5px;position:relative;z-index:1; }}
        .mod-desc {{ font-size:13px;color:#64748B;line-height:1.7;margin:0 0 22px;
            flex:1;position:relative;z-index:1; }}
        .mod-features {{ display:flex;flex-direction:column;gap:8px;margin-bottom:26px;position:relative;z-index:1; }}
        .mod-feat {{ display:flex;align-items:center;gap:8px;font-size:12px;color:#475569; }}
        .mod-feat-dot {{ width:6px;height:6px;border-radius:50%;background:var(--mc,{COLOR_PRIMARY});flex-shrink:0; }}

        /* Nav buttons */
        div[data-testid="stButton"] > button {{
            border-radius:12px!important; font-weight:700!important; font-size:14px!important;
            height:46px!important; transition:transform .18s ease,box-shadow .18s ease!important;
        }}
        div[data-testid="stButton"] > button[kind="primary"] {{
            background:linear-gradient(135deg,{COLOR_PRIMARY},{COLOR_ACCENT})!important;
            border:none!important; box-shadow:0 4px 14px rgba(14,165,233,0.30)!important;
        }}
        div[data-testid="stButton"] > button[kind="primary"]:hover {{
            transform:translateY(-2px)!important; box-shadow:0 8px 22px rgba(14,165,233,0.40)!important;
        }}
        div[data-testid="stButton"] > button[kind="secondary"] {{
            background:white!important; border:2px solid #E2E8F0!important; color:#64748B!important;
        }}
        div[data-testid="stButton"] > button[kind="secondary"]:hover {{
            border-color:#8B5CF6!important; color:#8B5CF6!important; transform:translateY(-2px)!important;
        }}

        /* Stats */
        .home-stats {{ display:flex;gap:16px;margin-top:36px;animation:fadeUp 0.9s ease both; }}
        .home-stat {{ flex:1;background:white;border-radius:16px;padding:22px 20px;
            box-shadow:0 4px 20px rgba(0,0,0,0.07);border:1px solid rgba(0,0,0,0.04);
            text-align:center;position:relative;overflow:hidden; }}
        .home-stat::before {{ content:'';position:absolute;top:0;left:0;right:0;height:3px;
            background:var(--sc,#94A3B8);border-radius:16px 16px 0 0; }}
        .home-stat-val {{ font-size:28px;font-weight:900;color:#1E293B;line-height:1;margin-bottom:4px; }}
        .home-stat-lbl {{ font-size:10px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.09em; }}

        /* Streamlit page nav links in sidebar */
    ...
    with col1:
        st.markdown("""
        <div class='mod-card' style='--mc:#0EA5E9'>
    ...
    with col2:
        st.markdown("""
        <div class='mod-card' style='--mc:#8B5CF6'>
            <div class='mod-card-wash'></div>
...
# ─────────────────────────────────────────────
# NAVEGACIÓN (API correcta de Streamlit 1.36+)
# ─────────────────────────────────────────────
home_pg = st.Page(home_content, title="Inicio", icon="🏠", default=True)
